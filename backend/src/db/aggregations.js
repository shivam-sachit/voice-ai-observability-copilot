...
// One row: how many analyses, their average score, and the total number of flagged use-actions.
...
export function analysisAggForAgent(agentId, db = getDb()) {
  return db.prepare(`
    SELECT COUNT(*)                                        AS analyzed,
           ...
           COALESCE(SUM(json_array_length(use_actions)), 0) AS useActionCount
